...
const searchReedJobs = async (req, res) => {
  ...
  try {
    ...
    const response = await fetch(
      `${process.env.REED_API_BASE_URL}/search?keywords=${encodeURIComponent(searchTerm)}&location=${encodeURIComponent(searchLocation)}&resultsToTake=10&resultsToSkip=${encodeURIComponent(skippedResults)}`,
      {
        method: "GET",
        headers: {
...
module.exports = { searchReedJobs, searchReedJob };
